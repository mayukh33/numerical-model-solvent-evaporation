"""Mixture reference state and the per-cell equation of state.  No FiPy."""
...
class Mixture:
    ...
    def __init__(self, component_A, component_B, wt_frac_A, *,
                 R, T, rho_gas, D_ref=None, D_bm=None, library=None):
        if D_ref is None or D_bm is None:
            if library is None:
                raise ValueError(
                    "Mixture needs liquid diffusivities: pass library=<your "
                    "ComponentLibrary>, or both D_ref and D_bm explicitly.")
            lookup_ref, lookup_bm = library.binary_diffusivities(
                component_A.name, component_B.name)
            D_ref = lookup_ref if D_ref is None else D_ref
            D_bm = lookup_bm if D_bm is None else D_bm
        ...
        self.alpha_A = component_A.alpha(D_ref)
        self.alpha_B = component_B.alpha(D_ref)
    ...
    def effective_liquid_diffusivity(self, x_A, x_B, delta):
        ...
        Dl = (self.D_ref ** x_B) * (self.D_bm ** x_A)
        return (Dl / self.D_ref) / delta
